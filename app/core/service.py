...
from .exceptions import CustomException, UnknownException, InvalidItemException, BadRequestException, CustomException, NotFoundException

S = TypeVar("S")
T = TypeVar("T")

class Service(Generic[S, T]):
    def __init__(self, repository: Repository, validate_item: Callable[[S], Result[S, CustomException]]):
