...
class CartRepository(Repository[CartItemIn, CartItem]):
    ...
    ) -> Result[List[CartItem], CustomException]:
        try:
            cursor = self.__db_conn.cursor()
            cursor.execute(
                "SELECT id, name, price, manufacturer, last_modified_at, created_at FROM cart LIMIT %s OFFSET %s",
    ...
    def get_item_by_id(self, id: str) -> Result[CartItem, CustomException]:
        try:
            cursor = self.__db_conn.cursor()
            cursor.execute(
                "SELECT id, name, price, manufacturer, last_modified_at, created_at FROM cart WHERE id = %s",
                [id],
            )
            row = cursor.fetchone()
            if row:
    ...
    def add_item(self, item: CartItemIn) -> Result[CartItem, CustomException]:
        try:
            cursor = self.__db_conn.cursor()
            cursor.execute(
                "INSERT INTO cart (name, price, manufacturer) VALUES (%s, %s, %s) RETURNING id, name, price, manufacturer, last_modified_at, created_at",
    ...
    def update_item(
        self, id: str, item: CartItemIn
    ) -> Result[CartItem, CustomException]:
        try:
            cursor = self.__db_conn.cursor()
            cursor.execute(
                "UPDATE cart SET name = %s, price = %s, manufacturer = %s WHERE id = %s RETURNING id, name, price, manufacturer, last_modified_at, created_at",
